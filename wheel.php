...
//  อัปเดตสถานะบังคับล้อแบบ AJAX 
if (isset($_POST['action']) && $_POST['action'] === 'unlock_status') {
  $offenderID = $_POST['offenderID'];

    // หา OffenderID ที่ตรงกับทุกค่า
    $stmt = $con->prepare( "UPDATE offense 
         SET Status='ปลดล็อคแล้ว' 
         WHERE OffenderID=?
    ");
...
// กรอง location
$location = [];
$sql = "SELECT l.`Location _Name` FROM location l where l.Location_ID like 'L0%'"; 
...
if (!empty($_POST['violation'])) {
    // implode ให้เป็น string เช่น "C002,C003"
    $category_id = implode(",", $_POST['violation']);
}
...
elseif ($len == 13) {
    ...
    $stmt = $con->prepare("INSERT INTO offense 
        ...
        VALUES (?, ?, ?, ?, ?, ?, 'รอปลดล็อค', NOW())");
    $stmt->bind_param("ssssss", 
        $newID, $category_id, $location_id, $officer_id, $date, $time
    );
    $stmt->execute();
    $stmt->close();
...
if ($resultCar->num_rows == 0) {
    $stmtIns = $con->prepare("
        ...
        VALUES (?, ?, ?, ?, ?)
    ");
    $stmtIns->bind_param("sssss", 
        $vehicle_num, $province, $motorcycle, $brand, $color
    );
    $stmtIns->execute();
    $stmtIns->close();
}
...
// ตรวจสอบการค้นหาเลขบัตรแบบ AJAX
...
if ($_SESSION['role'] === 'officer') {
    $whereOfficer = " AND o2.OfficerID = ? ";
    $types = "s";
    $params[] = $_SESSION['OfficerID'];
}


//ดึงข้อมูล ตารางรายการบันทึก
...
LEFT JOIN (
    SELECT o3.OffenderID, 
           GROUP_CONCAT(coo.Category_Name ORDER BY coo.Category_Name SEPARATOR ' , ') AS CategoryNames
    FROM offense o3
    JOIN `category _of_offense` coo  
         ON FIND_IN_SET(coo.CategoryID, o3.CategoryID)
    WHERE coo.CategoryID IN ('C002','C003','C004','C005','C006','C007','C008')
...
WHERE (s.Student_ID IS NOT NULL
  ...
  AND EXISTS (
       SELECT 1 
       FROM `category _of_offense` coo2
       WHERE FIND_IN_SET(coo2.CategoryID, o2.CategoryID)
         AND coo2.CategoryID IN ('C002','C003','C004','C005','C006','C007','C008')
  )
...
<head>
    ...
    * {
    ...
    input[type="radio"] {
        transform: scale(1.5);
        /* ปรับขนาดใหญ่ขึ้น 1.5 เท่า */
        margin-right: 10px;
        /* เว้นช่องว่างระหว่างปุ่มกับ label */
    }
...
<body>
    ...
    <div class="sidebar collapsed" id="sidebar">
        ...
        <div class="bottom-links">
            <a href="profile.php" class="nav-link">
                ...
                <span>
                    <?php echo htmlspecialchars($officerName); ?>
                </span>
            </a>
    ...
    <div class="content expanded" id="content">
        <header class="text-center mb-4">
            <h3>การบันทึกผู้กระทำความผิดระเบียบวินัยจราจร (บังคับล้อ)</h3>
        </header>
        ...
        <div class="form-box">
            <form id="offenseForm" method="post">
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label class="form-label d-block">ผู้กระทำผิด</label>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="type_offender" id="student"
                                value="นักศึกษา" required>
                            <label class="form-check-label" for="student">นักศึกษา</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="type_offender" id="personnel"
                                value="บุคลากร">
                            <label class="form-check-label" for="personnel">บุคลากร</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="type_offender" id="external"
                                value="บุคคลภายนอก">
                            <label class="form-check-label" for="external">บุคคลภายนอก</label>
                        </div>

                    </div>
                    <div class="col-md-4">
                        <label class="form-label">ประเภทรถ</label>
                        <select class="form-select" name="motorcycle" id="motorcycle">
                            <option value="" disabled selected hidden></option>
                ...
                <div class="row mb-3">
                    <div class="col-md-12">
                        ...
                        <?php
        $sql = "SELECT CategoryID, Category_Name 
                FROM `category _of_offense` 
        ...
        // แถวบน 3 ช่อง
        $topRow = array_slice($categories, 0, 3);
        // แถวกลาง 3 ช่อง
        $middleRow = array_slice($categories, 3, 3);
        // แถวล่าง 1 ช่อง
        $bottomRow = array_slice($categories, 6, 1);
        ?>

                        <!-- แถวบน -->
                        <div class="row mb-2">
                            <?php foreach ($topRow as $row) { ?>
                            <div class="col-md-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="violation[]"
                                        id="cat<?= $row['CategoryID'] ?>" value="<?= $row['CategoryID'] ?>">
                                    <label class="form-check-label" for="cat<?= $row['CategoryID'] ?>">
                                        <?= $row['Category_Name'] ?>
                                    </label>
                                </div>
                            </div>
...
?>
                            ...
                            <td><?php echo htmlspecialchars($row['CategoryNames']); ?></td>
                            <td><?php echo htmlspecialchars($row['Type_Vehicle']); ?></td>
                            <td><?php echo htmlspecialchars($row['Vehicle_Num']); ?></td>
                            <td><?php echo htmlspecialchars($row['Province']); ?></td>
                            <td><?php echo htmlspecialchars($row['Brand']); ?></td>
                            <td><?php echo htmlspecialchars($row['Color']); ?></td>
                            <td><?php echo htmlspecialchars($row['Location _Name']); ?></td>
                            <td>
                                <?php
...
?>
                            ...
                            <td>
                                <?php
                                    ...
                                    if ($status == 'รอปลดล็อค') {
                                        echo '<button 
    class="btn btn-status-lock btn-sm rounded-pill px-3 btn-status"
    data-id="'.$row['OffenderID'].'">
    <i class="bi bi-lock-fill"></i>
</button>';


                                    } elseif ($status == 'ปลดล็อคแล้ว') {
                                       echo '<button class="btn btn-success btn-sm rounded-pill px-3" disabled>
        <i class="bi bi-unlock-fill"></i>
      </button>';

                                    } else {
                                        echo '<button class="btn btn-secondary btn-sm rounded-pill px-3" disabled></button>';
                                    }
                                ?>
                            </td>
                            <td class="text-center">
                                <a href="edit_offense.php?id=<?= $row['OffenderID']; ?>" target="_blank"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        recordTable.addEventListener('click', function(e) {
            ...
            const offenderID = btn.dataset.id;
            const cardID = row.querySelector('td:nth-child(2)').textContent.trim();


            Swal.fire({
                title: 'ยืนยันการปลดล็อค?',
                text: `คุณต้องการปลดล็อคผู้กระทำผิดรหัส ${cardID} หรือไม่?`,
                icon: 'warning',
                showCancelButton: true,
                ...
                fetch(window.location.href, {
                        ...
                        body: `action=unlock_status&offenderID=${encodeURIComponent(offenderID)}`
                    })
                    .then(res => res.text())
                    .then(data => {
                        ...
                        if (data === 'OK') {
                            ...
                            // เปลี่ยนปุ่มทันที
                            btn.classList.remove('btn-status-lock', 'btn-status');
                            btn.classList.add('btn-success');
                            btn.disabled = true;
                            btn.innerHTML = '<i class="bi bi-unlock-fill"></i>';

                            Swal.fire('สำเร็จ!', 'ปลดล็อคเรียบร้อยแล้ว', 'success');
