...
<body>
    ...
    <div class="content expanded" id="content">
        <header class="text-center mb-4">
            <h3>รายการบันทึกผู้กระทำผิดระเบียบวินัยจราจร (บังคับล้อรถจักรยานยนต์)</h3>
            <?php
...
$enddate   = isset($_GET['enddate']) && $_GET['enddate'] !== '' ? $_GET['enddate'] : $today;
    ...
    l.`Location _Name`,
    o2.type_offender,
    COALESCE(s.Student_ID, p.Personnel_ID, o2.Identification_Num) AS Num,
    o2.Name,
    cat.CategoryNames,
    c.Type_Vehicle,
    c.Brand,
...
LEFT JOIN offender o2  
    ON o2.OffenderID = o.OffenderID 
LEFT JOIN student s 
    ON s.Student_ID = o2.Student_ID 
LEFT JOIN personnel p 
    ON p.Personnel_ID = o2.Personnel_ID 
LEFT JOIN (
    SELECT 
        o3.OffenderID, 
        GROUP_CONCAT(coo.Category_Name 
            ORDER BY coo.Category_Name 
            SEPARATOR ' , ') AS CategoryNames
    FROM offense o3
    JOIN `category _of_offense` coo 
        ON FIND_IN_SET(coo.CategoryID, o3.CategoryID)
    GROUP BY o3.OffenderID
) AS cat 
    ON cat.OffenderID = o2.OffenderID
LEFT JOIN car c 
    ON c.Vehicle_Num = o2.`Vehicle _Num`
WHERE c.Type_Vehicle = 'รถจักรยานยนต์'
  AND cat.CategoryNames <> 'ไม่สวมหมวกนิรภัย'
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>".htmlspecialchars($row['type_offender'])."</td>";
        echo "<td>".htmlspecialchars(maskText($row['Num'], 6))."</td>";
...
echo "<td>".htmlspecialchars(implode(" ", $maskedName))."</td>";
        echo "<td>".htmlspecialchars($row['CategoryNames'])."</td>";
