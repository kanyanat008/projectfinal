...
========================= */
$location = [];
$resLoc = $con->query("SELECT `Location _Name` FROM location WHERE Location_ID LIKE 'L0%'");
...
<head>
    <meta charset="UTF-8">
    <title>แก้ไขข้อมูลผู้กระทำผิด(บังคับล้อ)</title>
...
<body class="bg-light">
    ...
    <div class="container mt-4">
        <header class="text-center mb-4">
            <h3>แก้ไขข้อมูลผู้กระทำผิด(บังคับล้อ)</h3>
        </header>
        <div class="form-box">
            <form method="post">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select" name="motorcycle">
                            <option value="รถจักรยานยนต์" <?= $data['Type_Vehicle']=="รถจักรยานยนต์"?'selected':'' ?>>
                                รถจักรยานยนต์</option>
                            <option value="รถยนต์" <?= $data['Type_Vehicle']=="รถยนต์"?'selected':'' ?>>รถยนต์</option>
                        </select>
                    </div>
                </div>
                ...
                <label class="form-label fw-bold">ประเภทความผิด</label>
                <div class="row">
                    <?php
$resCat = $con->query("SELECT CategoryID, Category_Name FROM `category _of_offense` WHERE CategoryID<>'C001'");
                    ...
                    <div class="col-md-4">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="violation[]"
                                value="<?= $c['CategoryID'] ?>"
                                <?= in_array($c['CategoryID'], $selectedCategory) ? 'checked' : '' ?>>
